<?php	if ( ! defined('BASEPATH')) exit('No direct script access allowed'); 
	...
	*/
	function QueryByGivenDay( $dayDate ) {	
		...
		foreach( $staff as $Key => $employee) {
			...
			$QueryForShift = "SELECT s.startTime, s.endTime, s.Position, s.ShiftID FROM `Shifts` s JOIN `Employees` e ON s.CurrentOwnerEmployeeID = e.employeeID WHERE `date` =    '".$dayDate."' AND `CurrentOwnerEmployeeID` = '".$thisEmployeeID."' ORDER BY s.startTime ASC";			
			...
			foreach( $SHIFTS as $Shiftkey => $shift	) {
				...
				if ( $halfhour == 3 ) {  $halfhour = 1;  } else{  $halfhour = 0;  }
				$ST = ( $starthour * 2 ) + $halfhour;				
				$endhour = substr($ET, 0, 2);
				$endhour = (int)$endhour;
				$endhalf = substr($ET, 3, 1);
				if ( $endhalf == 3) {  $endhalf = 1;  } else{  $endhalf = 0;  }
  ...
  */
  public function CreateSelectArray() {
		...
		$days = 0;		
		$LISTofDATES = [			
			"2017"  =>  [ "January" => 0, "February" => 0, "March" => 0, "April" => 0, "May" => 0, "June" => 0, "July" => 0, "August" => 0, "September" => 0, "October" => 0, "November" => 0, "December" => 0 ],			
			"2018"	=> [ "January" => 0, "February" => 0, "March" => 0, "April" => 0, "May" => 0, "June" => 0, "July" => 0, "August" => 0, "September" => 0, "October" => 0, "November" => 0, "December" => 0 ]		
		];							
		...
		return json_encode($LISTofDATES); /* Return a fully populated array of days for each month in 2 years */		
